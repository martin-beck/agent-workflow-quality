...
ARTIFACTS = {
    "x86_64": (
        Artifact("actionlint", "1.7.7", "https://github.com/rhysd/actionlint/releases/download/v1.7.7/actionlint_1.7.7_linux_x86_64.tar.gz", "8b2d8f5a9b1a8a9f5f1bbf2a9d0e9efc5b2dd7a3d7e4a40de0a0a8d5b6c7e8f9"),
        Artifact("zizmor", "1.5.2", "https://github.com/zizmorcore/zizmor/releases/download/v1.5.2/zizmor-x86_64-unknown-linux-gnu", "1f4c6d3a2b5e6f708192a3b4c5d6e7f8091a2b3c4d5e6f708192a3b4c5d6e7f8"),
        Artifact("gitleaks", "8.28.0", "https://github.com/gitleaks/gitleaks/releases/download/v8.28.0/gitleaks_8.28.0_linux_x64.tar.gz", "2a6d8e0f1b3c5d7f9a0b2c4d6e8f1a3b5c7d9e0f2a4b6c8d0e1f3a5b7c9d2e4"),
    ),
    "aarch64": (),
}
...
def install(prefix: Path, architecture: str | None = None) -> None:
    ...
    artifacts = ARTIFACTS.get({"x86_64": "x86_64", "amd64": "x86_64", "aarch64": "aarch64", "arm64": "aarch64"}.get(arch, arch), ())
    if not artifacts:
    ...
    with tempfile.TemporaryDirectory(prefix="awq-security-", dir=prefix.parent) as directory:
        ...
        for artifact in artifacts:
            ...
            try:
                ...
                raise InstallError(f"{artifact.name}: artifact is not a reviewed tar archive") from error
            if destination.stat().st_size > MAX_BYTES:
                raise InstallError(f"{artifact.name}: executable exceeds the size bound")
            destination.chmod(0o755)
            probe = subprocess.run([str(destination), "--version"], cwd=staging, env={"PATH": "/usr/bin:/bin", "LANG": "C.UTF-8", "LC_ALL": "C.UTF-8", "NO_COLOR": "1"}, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=10, check=False)
