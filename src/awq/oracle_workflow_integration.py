...
def validate(value: Any) -> dict[str, Any]:  # noqa: C901
    ...
    for item, name, owner, evidence_class in zip(stages, _NAMES, _OWNERS, _CLASSES, strict=True):
        ...
        if (
            not isinstance(item["event_ref"], str)
            or not item["event_ref"].startswith("EV-")
            or not isinstance(item["task_revision"], int)
            or item["task_revision"] in revisions
            or item["task_revision"] < task["revision"]
        ):
    ...
    if (
        ...
        or set(decision) != {"disposition", "packet_ref", "decision_sha256"}
    ...
    if (
        ...
        or set(quality) != {"status", "evidence_ref", "requirement_sha256"}
...
def evaluate_file(root: Path, relative: str) -> dict[str, Any]:
    ...
    return {
        "status": "pass",
        "kind": "oracle-workflow-integration",
        "task": value["task"],
...
def check(root: Path, paths: list[Path]) -> list[dict[str, str]]:
    candidates = sorted(
        p
        for p in paths
        if p.relative_to(root).as_posix().startswith("quality/oracle-workflow-integration/")
        and p.suffix == ".json"
    )
    if not candidates:
        return [
            {
                "code": "missing-oracle-workflow-integration",
                "path": "quality/oracle-workflow-integration",
                "message": "no oracle workflow integration trace is declared",
            }
        ]
    ...
    for path in candidates[:128]:
        try:
            ...
            findings.append(
                {
                    "code": "invalid-oracle-workflow-integration",
