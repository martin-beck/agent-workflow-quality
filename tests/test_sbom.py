...
class SbomTests(unittest.TestCase):
    ...
    def test_inventory_scopes_origins_hashes_and_zero_runtime_are_explicit(self) -> None:
        ...
        self.assertEqual("0.17.0", packages["agent-workflow-quality"]["software_packageVersion"])
        self.assertTrue(
    ...
    def test_runtime_lock_origin_and_version_skew_fail(self) -> None:
        ...
        for old, new in (
            ...
            (b'version = "0.17.0"', b'version = "0.13.0"'),
            (b"sha256:", b"md5:"),
        ):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with tarfile.open(path, "w:gz") as archive:
                for name, raw in self.inputs.items():
                    member = tarfile.TarInfo("agent_workflow_quality-0.17.0/" + name)
                    member.size = len(raw)
                    archive.addfile(member, io.BytesIO(raw))
            self.assertEqual(self.inputs, sbom.archive_inputs(path, "0.17.0"))
            with self.assertRaises(ReleaseError):
                sbom.archive_inputs(path, "0.13.0")
            with mock.patch("awq.sbom.MAX_BYTES", 1), self.assertRaises(ReleaseError):
                sbom.archive_inputs(path, "0.17.0")
    ...
    def test_complete_offline_bundle_and_rehashed_hostile_sbom(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest_path = stage / "agent_workflow_quality-0.17.0.release.json"
    ...
    def write_wheel(path: Path, epoch: int) -> None:
        ...
        entries["agent_workflow_quality-0.17.0.dist-info/METADATA"] = (
